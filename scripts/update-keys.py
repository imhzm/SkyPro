...
out, err = run_command(ssh, '''mysql -u root -p'***REDACTED***' senderpro -e "UPDATE activation_keys SET status='active', expiry_date='2027-04-23';" 2>&1''')
...
# Verify
out, err = run_command(ssh, '''mysql -u root -p'***REDACTED***' senderpro -e "SELECT \`key\`, status, expiry_date FROM activation_keys;" 2>&1''')
print('Keys:')
print(out)
print()
