...
out, err = run_command(ssh, "mysql -u root -p'***REDACTED***' -e \"SHOW DATABASES LIKE 'senderpro';\"")
print(f'Database check: {out}')

if 'senderpro' not in out:
    print('Creating database senderpro...')
    out, err = run_command(ssh, "mysql -u root -p'***REDACTED***' -e \"CREATE DATABASE IF NOT EXISTS senderpro CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;\"")
    if err:
...
out, err = run_command(ssh, "mysql -u root -p'***REDACTED***' senderpro < /var/www/html/sender-pro-api/sender_pro_database.sql")
if err:
