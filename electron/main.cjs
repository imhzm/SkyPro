...
function saveAccount(acc) {
  if (globals.db) {
    try {
      globals.db.prepare("INSERT OR REPLACE INTO accounts (id, platform, username, password, status, cookies, proxy_id) VALUES (?, ?, ?, ?, ?, ?, ?)").run(acc.id, acc.platform, acc.username, acc.password, acc.status || 'active', acc.cookies || null, acc.proxy_id || null)
    } catch(e) {
...
ipcm('save-security-settings', async (e, settings) => {
  if (!db) return { success: false, error: 'قاعدة البيانات غير جاهزة' }
  try {
    // P2-32: Use atomic transaction so DELETE+INSERT can't leave empty table on crash
...
ipcm('run-tool', async (e, { platform, toolId, toolName, params, execute = false }) => {
  try {
    if (db) globals.db.prepare('INSERT INTO leads (platform, name, source, extra_data) VALUES (?, ?, ?, ?)')
...
async function executeCampaign(task) {
  try {
    ...
    if (db) globals.db.prepare("UPDATE campaigns SET status = ?, results = ? WHERE id = ?").run(result.status, JSON.stringify(result), task.id)
  } catch (err) {
    if (db) globals.db.prepare("UPDATE campaigns SET status = 'failed', results = ? WHERE id = ?").run(JSON.stringify({ error: err.message }), task.id)
  }
}
