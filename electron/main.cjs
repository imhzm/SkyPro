...
ipcm('close-browser', async (e, sessionId) => {
  try {
    return await bm.close(sessionId)
  } catch (err) {
    return { success: false, error: err.message }
  }
})
...
ipcm('get-browser-status', async (e, sessionId) => {
  try {
    return { success: true, active: bm.getBrowser(sessionId) !== undefined }
  } catch (err) {
    return { success: false, error: err.message }
  }
})
...
ipcm('check-platform-session', async (e, { platform, headless = false }) => {
  try {
    const res = await bm.launch({ headless, platform })
...
ipcm('facebook-login', async (e, { username, password, headless = false, proxy }) => {
  let sessionId = null
  try {
    const res = await bm.launch({ headless, platform: 'facebook', proxy: proxy || undefined })
...
ipcm('facebook-extract-likers', async (e, { sessionId, postUrl, limit = 50, jobId, delayMs = 2000 }) => {
  ...
  try {
    ...
    for (let i = 0; i < maxScrolls; i++) {
      if (cancelFlags.get(jobId)) break
    ...
    return { success: true, data: allUsers, count: allUsers.length, jobId, cancelled: cancelFlags.get(jobId) }
  } catch (err) {
...
ipcm('facebook-extract-comments', async (e, { sessionId, postUrl, limit = 50, jobId, delayMs = 2000 }) => {
  ...
  try {
    ...
    return { success: true, data: allComments, count: allComments.length, jobId, cancelled: cancelFlags.get(jobId) }
  } catch (err) {
...
ipcm('facebook-extract-group-members', async (e, { sessionId, groupUrl, limit = 50, jobId, delayMs = 2000 }) => {
  ...
  try {
    ...
    return { success: true, data: allMembers, count: allMembers.length, jobId, cancelled: cancelFlags.get(jobId) }
  } catch (err) {
...
ipcm('facebook-extract-friends', async (e, { sessionId, limit = 100, jobId, delayMs = 2000 }) => {
  ...
  try {
    ...
    return { success: true, data: allFriends, count: allFriends.length, jobId, cancelled: cancelFlags.get(jobId) }
  } catch (err) {
...
ipcm('facebook-extract-page-followers', async (e, { sessionId, pageUrl, limit = 50, jobId, delayMs = 2000 }) => {
  ...
  try {
    ...
    return { success: true, data: allFollowers, count: allFollowers.length, jobId, cancelled: cancelFlags.get(jobId) }
  } catch (err) {
...
ipcm('facebook-extract-phones', async (e, { sessionId, postUrl, limit = 50, jobId, delayMs = 2000 }) => {
  ...
  try {
    ...
    return { success: true, data: allPhones, count: allPhones.length, jobId, cancelled: cancelFlags.get(jobId) }
  } catch (err) {
...
ipcm('get-active-sessions', async () => {
  const sessions = []
  for (const [id, session] of bm.browsers) {
  ...
  while (!shouldStop) {
    ...
    for (let i = 0; i < accounts.length; i++) {
      ...
      try {
        ...
        for (const [existingId, session] of bm.browsers) {
          if (session.profileId === profileId) {
            try { await bm.close(existingId) } catch (e) { console.error('Error closing previous session:', e.message) }
            break
          }
        }
        ...
        for (const [existingId, session] of bm.browsers) {
          if (session.platform === platform && !session.profileId) {
            try { await bm.close(existingId) } catch (e) { console.error('Error closing legacy session:', e.message) }
            break
          }
        }

        // Launch browser with account-specific profile and proxy
        const launchRes = await bm.launch({ headless: false, platform, proxy: account.proxy || undefined, profileId })
        ...
        if (sessionId) {
          try { await bm.close(sessionId) } catch (e) { console.error('Error closing session:', e.message) }
          sessionId = null
        }
      }

      // Check if user requested stop
      if (cancelFlags.get('cycle-stop')) {
        cancelFlags.delete('cycle-stop')
        shouldStop = true
        break
      }
...
ipcm('stop-cycle', async () => {
  cancelFlags.set('cycle-stop', true)
...
ipcm('whatsapp-launch', async (e, { proxy } = {}) => {
  let sessionId = null
  try {
    const res = await bm.launch({ headless: false, platform: 'whatsapp', proxy })
    if (!res.success) return res
    sessionId = res.sessionId
    const page = bm.getPage(sessionId)
...
ipcm('whatsapp-extract-groups', async (e, { sessionId }) => {
  ...
  try {
    ...
    groups.forEach(g => { if (db) db.prepare('INSERT INTO leads (platform, name, source, extra_data) VALUES (?, ?, ?, ?)').run('whatsapp', g.name, 'groups', g.extra) })
    return { success: true, data: groups, count: groups.length }
  } catch (err) {
    return { success: false, error: err.message }
  }
...
ipcm('whatsapp-filter-numbers', async (e, { numbers }) => {
  let sessionId = null
  try {
    const res = await bm.launch({ headless: true, platform: 'whatsapp' })
    if (!res.success) return res
    sessionId = res.sessionId
    const page = bm.getPage(sessionId)
    const results = []
...
ipcm('instagram-login', async (e, { username, password, headless = false, proxy }) => {
  let sessionId = null
  try {
    const res = await bm.launch({ headless, platform: 'instagram', proxy: proxy || undefined })
    if (!res.success) return res
    sessionId = res.sessionId
    const page = bm.getPage(sessionId)
...
ipcm('twitter-login', async (e, { username, password, headless = false, proxy }) => {
  let sessionId = null
  try {
    const res = await bm.launch({ headless, platform: 'twitter', proxy: proxy || undefined })
    if (!res.success) return res
    sessionId = res.sessionId
    const page = bm.getPage(sessionId)
...
ipcm('twitter-schedule-tweet', async (e, { text, scheduledAt }) => {
  ...
  try {
    const stmt = db.prepare('INSERT INTO campaigns (name, platform, type, status, results, scheduled_at, data) VALUES (?, ?, ?, ?, ?, ?, ?)')
...
ipcm('linkedin-login', async (e, { username, password, headless = false, proxy }) => {
  let sessionId = null
  try {
    const res = await bm.launch({ headless, platform: 'linkedin', proxy: proxy || undefined })
    if (!res.success) return res
    sessionId = res.sessionId
    const page = bm.getPage(sessionId)
...
ipcm('telegram-login', async (e, { phoneNumber, headless = false }) => {
  let sessionId = null
  try {
    const res = await bm.launch({ headless, platform: 'telegram' })
    if (!res.success) return res
    sessionId = res.sessionId
    const page = bm.getPage(sessionId)
...
ipcm('pinterest-login', async (e, { username, password, headless = false, proxy }) => {
  let sessionId = null
  try {
    const res = await bm.launch({ headless, platform: 'pinterest', proxy: proxy || undefined })
    if (!res.success) return res
    sessionId = res.sessionId
    const page = bm.getPage(sessionId)
...
ipcm('threads-login', async (e, { username, password, headless = false, proxy }) => {
  let sessionId = null
  try {
    const res = await bm.launch({ headless, platform: 'threads', proxy: proxy || undefined })
    if (!res.success) return res
    sessionId = res.sessionId
    const page = bm.getPage(sessionId)
...
ipcm('reddit-login', async (e, { username, password, headless = false, proxy }) => {
  let sessionId = null
  try {
    const res = await bm.launch({ headless, platform: 'reddit', proxy: proxy || undefined })
    if (!res.success) return res
    sessionId = res.sessionId
    const page = bm.getPage(sessionId)
...
ipcm('google-maps-extract', async (e, { searchQuery, location, limit = 50, headless = false, sessionId: existingSessionId }) => {
  ...
  try {
    if (!sessionId) {
      const res = await bm.launch({ headless: true, platform: 'google-maps' })
      if (!res.success) return res
      sessionId = res.sessionId
    }
    const page = bm.getPage(sessionId)
    ...
    if (ownSession) await bm.close(sessionId)
...
ipcm('olx-extract', async (e, { country, category, limit = 50, sessionId: existingSessionId }) => {
  ...
  try {
    if (!sessionId) {
      const res = await bm.launch({ headless: true, platform: 'olx' })
      if (!res.success) return res
      sessionId = res.sessionId
    }
    const page = bm.getPage(sessionId)
...
ipcm('send-email', async (e, { provider, username, password, to, subject, body, headless = false }) => {
  let sessionId = null
  try {
    const res = await bm.launch({ headless: false, platform: 'gmail' })
    if (!res.success) return res
    sessionId = res.sessionId
    const page = bm.getPage(sessionId)
    if (provider === 'gmail') {
...
ipcm('auto-point-run', async (e, { platform, site, interactionType, count, delay, headless = false }) => {
  let sessionId = null
  try {
    const res = await bm.launch({ headless, platform: 'auto-point' })
    if (!res.success) return res
    sessionId = res.sessionId
    const page = bm.getPage(sessionId)
    let url = site
...
ipcm('video-download', async (e, { url, saveDir }) => {
  ...
  try {
    ...
    const res = await bm.launch({ headless: true, platform: 'video-download' })
    if (!res.success) return res
    sessionId = res.sessionId
    const page = bm.getPage(sessionId)
...
ipcm('generate-hashtags', async (e, { keyword, platform }) => {
  let sessionId = null
  try {
    const res = await bm.launch({ headless: true })
    if (!res.success) return res
    sessionId = res.sessionId
    const page = bm.getPage(sessionId)
    let hashtags = []
...
ipcm('test-proxy', async (e, { proxy }) => {
  let sessionId = null
  try {
    const res = await bm.launch({ headless: true, proxy })
    if (!res.success) return res
    sessionId = res.sessionId
    const page = bm.getPage(sessionId)
...
ipcm('snapchat-login', async (e, { username, password, headless = false, proxy }) => {
  let sessionId = null
  try {
    const res = await bm.launch({ headless, platform: 'snapchat', proxy: proxy || undefined })
    if (!res.success) return res
    sessionId = res.sessionId
    const page = bm.getPage(sessionId)
...
ipcm('snapchat-broadcast', async (e, { sessionId, usernames, message }) => {
  try {
    const page = sessionId ? bm.getPage(sessionId) : null
    if (page) {
    ...
    try {
      const res = await bm.launch({ headless: false, platform: 'snapchat' })
      if (!res.success) return res
      newSessionId = res.sessionId
      const newPage = bm.getPage(newSessionId)
...
ipcm('delete-smtp-setting', async (e, { id }) => {
  ...
  if (!Number.isInteger(id) || id < 1) return { success: false, error: 'Invalid id' }
  try {
    db.prepare('DELETE FROM smtp_settings WHERE id = ?').run(id)
    return { success: true }
  } catch (err) {
    return { success: false, error: err.message }
  }
...
ipcm('save-security-settings', async (e, settings) => {
  if (!db) return { success: false, error: 'قاعدة البيانات غير جاهزة' }
  try {
    db.exec('DELETE FROM security_settings')
    db.prepare('INSERT INTO security_settings (enabled, randomDelays, minDelay, maxDelay, maxActionsPerHour, rotateUserAgent, randomizeViewport, useStealthMode, maxRetries) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)').run(
...
ipcm('get-scheduled-campaigns', async () => {
  if (!db) return { success: false, data: [], error: 'قاعدة البيانات غير جاهزة' }
  try {
...
ipcm('export-csv', async (e, { filename, data, headers }) => {
  try {
    ...
    const rows = [headers.join(','), ...data.map(row => headers.map(h => `"${(row[h] || '').toString().replace(/"/g, '""')}"`).join(','))]
...
ipcm('export-excel', async (e, { filename, data, headers }) => {
  try {
    ...
    if (result.canceled) return { success: false, error: 'تم الإلغاء' }
    const headerRow = '<tr>' + headers.map(h => `<th style="background:#2563eb;color:white;font-weight:bold;padding:8px;border:1px solid #ddd;">${h}</th>`).join('') + '</tr>'
    const dataRows = data.map(row => '<tr>' + headers.map(h => `<td style="padding:8px;border:1px solid #ddd;">${(row[h] || '').toString().replace(/</g, '&lt;').replace(/>/g, '&gt;')}</td>`).join('') + '</tr>').join('\n')
...
ipcm('run-tool', async (e, { platform, toolId, toolName, params, execute = false }) => {
  try {
    if (db) db.prepare('INSERT INTO leads (platform, name, source, extra_data) VALUES (?, ?, ?, ?)')
...
async function executeCampaign(task) {
  try {
    ...
    switch (task.platform) {
      case 'twitter':
        if (data.text) {
          const acc = db ? db.prepare("SELECT * FROM accounts WHERE platform = 'twitter' ORDER BY id DESC LIMIT 1").get() : null
          if (acc) {
    ...
    if (db) db.prepare("UPDATE campaigns SET status = ?, results = ? WHERE id = ?").run(result.status, JSON.stringify(result), task.id)
  } catch (err) {
    if (db) db.prepare("UPDATE campaigns SET status = 'failed', results = ? WHERE id = ?").run(JSON.stringify({ error: err.message }), task.id)
  }
}
...
setInterval(async () => {
  if (!db) return
  try {
    const tasks = db.prepare("SELECT * FROM campaigns WHERE status = 'pending' AND datetime(scheduled_at) <= datetime('now')").all()
...
app.on('before-quit', () => {
  if (bm) { try { bm.closeAll() } catch (e) { console.error('closeAll error:', e.message) } }
  if (db) { try { db.close(); db = null } catch (e) { console.error('db.close error:', e.message) } }
})
...
function sendProgress(sender, jobId, data) {
  try { sender.send('extraction-progress', { jobId, ...data }) } catch (e) { console.error('sendProgress error:', e.message) }
}
