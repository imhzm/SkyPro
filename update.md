...
# Check user status
mysql -u skypro_app -p'***REDACTED***' skypro -e "SELECT id, email, status, email_verified_at FROM users WHERE email='USER_EMAIL';"

# If status is 'pending_verification', the user hasn't verified their email
# To manually activate:
mysql -u skypro_app -p'***REDACTED***' skypro -e "UPDATE users SET status='active', email_verified_at=NOW() WHERE email='USER_EMAIL';"

# Check activation key
mysql -u skypro_app -p'***REDACTED***' skypro -e "SELECT id, key_code, status, user_id, expires_at FROM activation_keys WHERE user_id=(SELECT id FROM users WHERE email='USER_EMAIL');"

# If key status is 'pending', activate it:
mysql -u skypro_app -p'***REDACTED***' skypro -e "UPDATE activation_keys SET status='active', activated_at=NOW(), expires_at=DATE_ADD(NOW(), INTERVAL 1 YEAR) WHERE user_id=(SELECT id FROM users WHERE email='USER_EMAIL') AND status='pending';"
```
